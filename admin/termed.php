<?php
    ...
    try{
        ...
        $consulta  ="SELECT ee.eeid, ea.no_serial, ee.fecha_terminacion FROM equip_assignment as ea JOIN empleados as ee on ee.eeid=ea.eeid WHERE ee.fecha_terminacion != ''";
    ...
    if (isset($_POST['submit']) && isset($_POST['busqueda']) && $_POST['busqueda']!=''){
        ...
        $database = new Database();
        $consulta  = "SELECT ee.eeid, ea.no_serial, ee.fecha_terminacion FROM equip_assignment as ea JOIN empleados as ee on ee.eeid=ea.eeid WHERE  (ee.eeid LIKE '%".$search."%' OR ea.no_serial LIKE '%".$search."%' OR ee.fecha_terminacion LIKE '%".$search."%');";
        $datos = $database->conectar()->prepare($consulta);
        $datos->execute();
        $resultado = $datos->fetchAll();
        }catch(PDOException $Error) {$mensaje = "Ocurrió un error al cargar los registros."; }
...
<body>
    ...
    <div class="container-fluid px-3 py-2 px-md-5">
        <div class="d-flex bd-highlight text-center">
            ...
            <div class="p-2 w-100">
                <h1 class="display-6">Termed Employees with Equipment</h1>
            </div>
        </div>
